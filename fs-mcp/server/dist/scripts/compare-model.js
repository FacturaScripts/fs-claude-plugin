...
function parseArgs() {
    ...
    if (!args['model'])
        die('Falta --model=<nombre_modelo>');
    if (!args['source'])
        die('Falta --source=<ruta>');
...
function parseTableXml(xml) {
    ...
    for (const m of xml.matchAll(/<constraint>([\s\S]*?)<\/constraint>/g)) {
        ...
        const fk = def.match(/^FOREIGN\s+KEY\s*\(([^)]+)\)\s+REFERENCES\s+(\w+)\s*\(([^)]+)\)/i);
        if (fk?.[1] && fk[2] && fk[3]) {
...
// ───────────────────────────────────────────────────────────────────────────
function findTableXmlPath(sourceRoot, type, modelName) {
 ...
 */
function candidateTableNames(modelName) {
...
function suggestEditViewName(modelName) {
    return 'Edit' + modelName.split('_').map((p) => p[0]?.toUpperCase() + p.slice(1)).join('');
}
function suggestEndpoint(tableName) {
...
async function main() {
    ...
    if (resolvedTable)
        output.sourceTable = resolvedTable;
    if (currentMeta)
        output.registryTable = currentMeta.table;
    ...
    if (!inRegistry) {
        ...
        if (existsSync(editViewPath))
            tableEntry.editViewHint = editView;
    ...
    for (const [name, src] of sourceByName) {
        ...
        if (diffs.length > 0)
            output.changed.push(...diffs);
        else
            output.unchanged.push(name);
    }
    for (const [name, cur] of currentByName) {
        if (!sourceByName.has(name))
            output.removed.push(cur);
    }
    output.summary = {
