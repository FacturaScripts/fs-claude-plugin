    ...
    for (const meta of models) {
        ...
        dump[`${meta.name} (${meta.table}, ${meta.source})`] = undoc.map((c) => {
            ...
            if (c.isRequired)
                out.required = true;
            if (c.foreignKey)
                out.fk = `${c.foreignKey.table}.${c.foreignKey.column}`;
            if (c.enumValues)
                out.enum = c.enumValues;
            return out;
        });
    }
    ...
    for (const meta of models) {
        ...
        for (const col of undoc) {
            ...
            if (col.isPrimaryKey)
                tags.push('PK');
            if (col.foreignKey)
                tags.push(`FK→${col.foreignKey.table}`);
            if (col.isReadonly)
                tags.push('RO');
            if (col.isRequired)
                tags.push('required');
            ...
            reportLines.push(`  - ${col.name} (${col.tsType}${col.maxLength ? `(${col.maxLength})` : ''})${tagStr}${enumStr} — label="${col.label}"`);
        }
    }
...
//# sourceMappingURL=list-undocumented.js.map