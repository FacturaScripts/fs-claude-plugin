...
function parseArgs() {
    ...
    const result = {};
    if (args['fs-path'])
        result.fsPath = resolve(args['fs-path']);
    if (args['manifest'])
        result.manifest = resolve(args['manifest']);
    if (args['only']) {
...
function parseEditViewXml(xml) {
    ...
    for (const block of columnBlocks) {
        ...
        if (descMatch?.[1])
            info.descriptionKey = descMatch[1];
        if (typeMatch?.[1])
            info.widget = normalizeWidgetType(typeMatch[1]);
        if (readonlyMatch?.[1] && readonlyMatch[1] !== 'false')
            info.readonly = true;
        if (requiredMatch)
            info.required = true;
        if (maxLengthMatch?.[1])
            info.maxLength = Number(maxLengthMatch[1]);
        if (enumValues.length > 0)
            info.enumValues = enumValues;
...
function normalizeWidgetType(raw) {
    ...
    return (known.includes(raw) ? raw : 'text');
}
...
function sqlTypeToTs(sqlType) {
    ...
    if (lower === 'date')
        return { tsType: 'date' };
    if (lower === 'timestamp' || lower.startsWith('timestamp'))
        return { tsType: 'datetime' };
    if (lower === 'time')
        return { tsType: 'time' };
    if (lower === 'text')
        return { tsType: 'text' };
...
function parseDefault(raw, tsType) {
...
// ───────────────────────────────────────────────────────────────────────────
function buildColumnMetadata(col, tableDef, viewField, translations, descriptionOverride) {
    ...
    if (resolvedMaxLength !== undefined)
        result.maxLength = resolvedMaxLength;
...
function resolveLabel(columnName, viewField, translations) {
...
function resolveDescription(columnName, viewField, translations, fk) {
...
// ───────────────────────────────────────────────────────────────────────────
function buildRelations(modelTable, tableDef, allTables, catalog) {
    const relations = [];
...
async function writeIndexFile(outDir, names) {
...
async function mergeExtensionTables(tables, pluginDir) {
    ...
    for (const file of files) {
        ...
        if (!target)
            continue; // solo extendemos tablas ya conocidas
 ...
 */
async function loadCombinedTranslations(fsPath, extraJsonPath) {
    const corePath = join(fsPath, 'Core', 'Translation', 'es_ES.json');
    const dict = {};
    if (existsSync(corePath)) {
        Object.assign(dict, JSON.parse(await readFile(corePath, 'utf8')));
    }
    if (extraJsonPath && existsSync(extraJsonPath)) {
        Object.assign(dict, JSON.parse(await readFile(extraJsonPath, 'utf8')));
    }
 ...
 */
async function loadDescriptionsOverrides(path) {
    ...
    try {
        ...
        console.warn(`[generate-metadata] Error cargando overrides desde ${path}: ${err.message}`);
        return {};
    }
}
/**
...
// Modo CORE
// ───────────────────────────────────────────────────────────────────────────
async function runCoreMode(args) {
...
// Modo PLUGIN
// ───────────────────────────────────────────────────────────────────────────
async function runPluginMode(args) {
    ...
    const pluginCatalog = models.map((m) => ({
        name: m.name,
        table: m.table,
        endpoint: m.endpoint,
    ...
    const fullCatalog = [...MODEL_CATALOG, ...pluginCatalog];
    const fsCommit = resolveGitCommit(fsPath);
    ...
    for (const m of models) {
        ...
        if (fsCommit)
            generatedFrom.facturascriptsCommit = fsCommit;
        if (pluginCommit)
            generatedFrom.facturascriptsVersion = `plugin:${moduleName}@${pluginCommit}`;
...
function validateManifest(m) {
    const required = ['moduleName', 'fsPath', 'pluginPath', 'outputBase', 'models'];
...
async function loadEditViewIfExists(viewDir, editView, modelName) {
    if (!editView)
        return new Map();
...
function reportMissedTranslations(translations) {
    if (translations.missed.size === 0)
        return;
 ...
 */
function serializeMetadataAsTs(meta) {
 ...
 */
function serializeMetadataAsJs(meta) {
    const json = JSON.stringify(meta, null, 4);
    const varName = toCamelCase(meta.name) + 'Metadata';
