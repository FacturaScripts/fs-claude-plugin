...
function nuevoAgregadoMonetario() {
    return {
        totalFacturado: 0,
        totalCoste: 0,
        totalBeneficio: 0,
...
function acumularFactura(agg, factura) {
...
function calcularMargenes(agregados) {
...
// ============================================================================
export const extendedAnalyticsTools = [
    // ---- VENTAS ----
    {
...
// Handlers
// ============================================================================
export async function handleExtendedAnalyticsTool(name, args) {
    const input = args;
        ...
        switch (name) {
            ...
            case 'get_ventas_por_agente': {
                ...
                for (const f of facturas) {
                    ...
                    if (!map[cod])
                        map[cod] = { codagente: cod, ...nuevoAgregadoMonetario() };
            ...
            case 'get_ventas_por_serie': {
                ...
                for (const f of facturas) {
                    ...
                    if (!map[cod])
                        map[cod] = { codserie: cod, ...nuevoAgregadoMonetario() };
                    acumularFactura(map[cod], f);
                }
                const result = Object.values(map);
                calcularMargenes(result);
            ...
            case 'get_ventas_por_almacen': {
                ...
                for (const f of facturas) {
                    ...
                    if (!map[cod])
                        map[cod] = { codalmacen: cod, ...nuevoAgregadoMonetario() };
                    acumularFactura(map[cod], f);
                }
                const result = Object.values(map);
                calcularMargenes(result);
            ...
            case 'get_ventas_por_forma_pago': {
                ...
                for (const f of facturas) {
                    ...
                    if (!map[cod])
                        map[cod] = { codpago: cod, ...nuevoAgregadoMonetario() };
                    acumularFactura(map[cod], f);
                }
                const result = Object.values(map);
                calcularMargenes(result);
            ...
            case 'get_ventas_por_familia': {
                ...
                for (const r of result)
                    r.margenPorcentaje = porcentaje(r.beneficio, r.importe);
            ...
            case 'get_productos_top_margen': {
                ...
                for (const p of all)
                    p.margenPorcentaje = porcentaje(p.beneficio, p.importe);
            ...
            case 'get_funnel_ventas': {
                ...
                const [presupuestos, pedidos, albaranes, facturas] = await Promise.all([
                    fetchAllPaginated('/presupuestoclientes', {}, connection),
                    fetchAllPaginated('/pedidoclientes', {}, connection),
                    fetchAllPaginated('/albaranclientes', {}, connection),
            ...
            case 'get_presupuestos_pendientes': {
                ...
                for (const r of result) {
                    ...
                    if (!porAgente[cod])
                        porAgente[cod] = { codagente: cod, cantidad: 0, importeTotal: 0 };
            ...
            case 'get_pedidos_pendientes_servir': {
                ...
                const pedidos = await fetchAllPaginated('/pedidoclientes', { editable: true }, connection);
                const now = new Date();
            ...
            case 'get_albaranes_pendientes_facturar': {
                ...
                const albaranes = await fetchAllPaginated('/albaranclientes', { editable: true }, connection);
                const now = new Date();
            ...
            case 'get_top_proveedores_compras': {
                ...
                for (const f of facturas) {
                    ...
                    if (!map[cod])
                        map[cod] = { codproveedor: cod, nombre: f.nombre ?? '', totalGastado: 0, numeroFacturas: 0 };
            ...
            case 'get_compras_por_familia': {
                ...
                for (const l of lineas) {
                    const codfamilia = familiaPorReferencia.get(l.referencia) ?? '(sin familia)';
                    if (!map[codfamilia])
                        map[codfamilia] = { codfamilia, cantidad: 0, gasto: 0 };
            ...
            case 'get_pagos_pendientes_proveedor': {
                ...
                const [recibos, proveedores] = await Promise.all([
                    fetchAllPaginated('/reciboproveedores', { pagado: false }, connection),
                    fetchAllPaginated('/proveedores', {}, connection),
                ]);
                const proveedorPorCodigo = new Map();
                for (const p of proveedores)
                    proveedorPorCodigo.set(p.codproveedor, p);
                const now = new Date();
                const map = {};
            ...
            case 'get_aging_cobros': {
                ...
                const recibos = await fetchAllPaginated('/reciboclientes', { pagado: false }, connection);
                const now = new Date();
                const buckets = {
                ...
                for (const r of recibos) {
                    ...
                    if (dias <= 0) {
                        buckets.alDia.cantidad++;
                        buckets.alDia.importe += importe;
                    }
                    else if (dias <= 30) {
                        buckets.de1a30.cantidad++;
                        buckets.de1a30.importe += importe;
                    }
                    else if (dias <= 60) {
                        buckets.de31a60.cantidad++;
                        buckets.de31a60.importe += importe;
                    }
                    else if (dias <= 90) {
                        buckets.de61a90.cantidad++;
                        buckets.de61a90.importe += importe;
                    }
                    else {
                        buckets.masDe90.cantidad++;
                        buckets.masDe90.importe += importe;
                    }
                }
            ...
            case 'get_aging_pagos': {
                ...
                const recibos = await fetchAllPaginated('/reciboproveedores', { pagado: false }, connection);
                const now = new Date();
                const buckets = {
                    alDia: { rango: 'No vencido', cantidad: 0, importe: 0 },
                    de1a30: { rango: '1-30 días', cantidad: 0, importe: 0 },
            ...
            case 'get_dso': {
                ...
                const recibos = await fetchAllPaginated('/reciboclientes', { pagado: true }, connection);
                let totalDias = 0;
                let totalRecibos = 0;
                const porCliente = {};
                for (const r of recibos) {
                    ...
                    if (!porCliente[cod])
                        porCliente[cod] = { codcliente: cod, sumaDias: 0, numero: 0, dsoPromedio: 0 };
                ...
                for (const c of detalleClientes)
                    c.dsoPromedio = c.numero > 0 ? c.sumaDias / c.numero : 0;
            ...
            case 'get_cash_flow_proyectado': {
                ...
                for (const r of cobrosPendientes) {
                    ...
                    if (!semanas[k])
                        semanas[k] = { semana: k, cobros: 0, pagos: 0, neto: 0 };
            ...
            case 'get_valoracion_inventario': {
                ...
                const stockParams = {};
                if (codalmacen)
                    stockParams.codalmacen = codalmacen;
                const [stocks, variantes, productos] = await Promise.all([
                    fetchAllPaginated('/stocks', stockParams, connection),
                    fetchAllPaginated('/variantes', {}, connection),
                    fetchAllPaginated('/productos', {}, connection),
                ]);
                // Coste por referencia (primera variante encontrada para esa referencia)
                const costePorReferencia = new Map();
                ...
                for (const s of stocks) {
                    ...
                    if (!porAlmacen[alm])
                        porAlmacen[alm] = { codalmacen: alm, cantidadTotal: 0, valorCoste: 0 };
                    ...
                    if (!porFamilia[fam])
                        porFamilia[fam] = { codfamilia: fam, cantidadTotal: 0, valorCoste: 0 };
            ...
            case 'get_rotacion_stock': {
                ...
                const [stocks, lineas, facturas] = await Promise.all([
                    fetchAllPaginated('/stocks', {}, connection),
                ...
                result.sort((a, b) => {
                    ...
                    if (a.diasCoberturaEstimados === null && b.diasCoberturaEstimados !== null)
                        return -1;
                    if (b.diasCoberturaEstimados === null && a.diasCoberturaEstimados !== null)
                        return 1;
            ...
            case 'get_stock_muerto': {
                ...
                for (const l of lineas) {
                    if (facturasEnRango.has(l.idfactura))
                        referenciasVendidas.add(l.referencia);
                }
                const productoPorReferencia = new Map();
                for (const p of productos)
                    productoPorReferencia.set(p.referencia, p);
                const stockPorReferencia = {};
                for (const s of stocks) {
                    stockPorReferencia[s.referencia] = (stockPorReferencia[s.referencia] || 0) + (s.cantidad || 0);
                }
            ...
            case 'get_clientes_lifetime_value': {
                ...
                for (const c of clientes)
                    clientePorCodigo.set(c.codcliente, c);
                const map = {};
                for (const f of facturas) {
                    const cod = f.codcliente;
                    let agg = map[cod];
            ...
            case 'get_clientes_riesgo_credito': {
                ...
                const clientes = await fetchAllPaginated('/clientes', {}, connection);
...
//# sourceMappingURL=kpis-extended.js.map