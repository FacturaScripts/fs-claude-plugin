...
function buildSchemaTools() {
    ...
    return [
        {
            name: 'describe_model',
            description: 'Devuelve la metadata completa de un modelo de FacturaScripts (columnas, tipos, relaciones, descripciones). ' +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    format: {
                        type: 'string',
                        description: 'Formato del resultado. "json" (por defecto) para procesamiento automático, "markdown" para lectura humana.',
                        enum: ['json', 'markdown'],
                    },
                },
                required: ['model'],
        ...
        {
            name: 'list_models',
            description: 'Lista todos los modelos de FacturaScripts (core + módulos locales privados) con metadata disponible, con una breve descripción de cada uno. ' +
        ...
        },
        {
            ...
            description: 'Verifica que las columnas declaradas en la metadata local coinciden con las que devuelve la API REST de la instalación de FacturaScripts. ' +
...
export async function registerSchemaTools(tools) {
    for (const tool of buildSchemaTools()) {
        tools.set(tool.name, tool);
    }
}
export async function handleSchemaTool(name, input) {
    if (name === 'list_models') {
    ...
    if (name === 'verify_model_columns') {
        ...
        try {
            const response = await fsClient.get(meta.endpoint, { limit: 1 }, connection);
 ...
 */
function extractFirstRecord(response) {
