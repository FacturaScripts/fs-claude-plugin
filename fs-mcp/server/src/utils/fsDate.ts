...
const FECHA_INVALIDA = (): Date => new Date(NaN);
...
function construir(
    ...
    if (mes < 1 || mes > 12 || dia < 1 || dia > 31) return FECHA_INVALIDA();

    const d = new Date(anio, mes - 1, dia, hora, minuto, segundo);
    const desbordado =
        d.getFullYear() !== anio || d.getMonth() !== mes - 1 || d.getDate() !== dia;
 ...
 */
export function parseFsDate(value: unknown): Date {
    if (value instanceof Date) return value;
    if (typeof value !== 'string') return FECHA_INVALIDA();

    const texto = value.trim();
    if (texto === '') return FECHA_INVALIDA();

    const dmy = RE_DMY.exec(texto);
    if (dmy) {
        return construir(
            num(dmy[3]), num(dmy[2]), num(dmy[1]),
            num(dmy[4]), num(dmy[5]), num(dmy[6]),
        );
    }
    ...
    if (iso) {
        return construir(
            num(iso[1]), num(iso[2]), num(iso[3]),
            num(iso[4]), num(iso[5]), num(iso[6]),
        );
    }
