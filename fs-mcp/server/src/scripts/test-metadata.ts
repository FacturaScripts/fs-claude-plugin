...
const modelMetadata: Record<string, ReturnType<typeof registryGetMeta>> = {};
for (const n of modelNames) modelMetadata[n] = registryGetMeta(n);
...
function assert(name: string, cond: boolean, detail?: string): void {
    const entry: TestResult = { name, ok: cond };
...
assert(
    `hay ${EXPECTED_MODEL_COUNT} modelos en modelMetadata`,
    modelNames.length === EXPECTED_MODEL_COUNT,
    `modelos: ${modelNames.length}`,
);
...
assert(
    `listSchemaResources devuelve 1 + ${EXPECTED_MODEL_COUNT}×3 = ${expectedResources} resources`,
    resources.length === expectedResources,
    `encontrados: ${resources.length}`,
);
assert(
    'primer resource es fs-schema://models',
    resources[0]?.uri === 'fs-schema://models',
);
...
if (indexResource) {
    const parsed = JSON.parse(indexResource.text) as { models: Array<{ name: string; columnsCount: number }> };
    assert(
        `índice contiene ${EXPECTED_MODEL_COUNT} modelos`,
        parsed.models.length === EXPECTED_MODEL_COUNT,
        `${parsed.models.length} modelos`,
    );
    const clienteEntry = parsed.models.find((m) => m.name === 'cliente');
    assert(
        'cliente en el índice tiene columnsCount > 0',
        (clienteEntry?.columnsCount ?? 0) > 0,
        `columnas: ${clienteEntry?.columnsCount}`,
    );
}
...
if (clienteResource) {
    assert(
        'mimeType de cliente JSON es application/json',
        clienteResource.mimeType === 'application/json',
    );
    const parsed = JSON.parse(clienteResource.text) as { name: string; table: string; primaryKey: string; columns: unknown[] };
...
if (clienteMd) {
    assert(
        'mimeType de cliente.md es text/markdown',
        clienteMd.mimeType === 'text/markdown',
    );
...
if (asientoRels) {
    const parsed = JSON.parse(asientoRels.text) as { relations: Array<{ type: string }> };
    assert(
        'asiento tiene relaciones',
...
try {
    ...
    assert('modelo inexistente lanza error', true, (err as Error).message);
}
...
if (listToolResult) {
    ...
    const parsed = JSON.parse(text) as { models: Array<{ name: string }> };
...
if (descJson && !descJson.isError) {
    const parsed = JSON.parse(descJson.content[0].text) as { name: string; columns: unknown[]; relations: unknown[] };
    assert('describe_model producto.name correcto', parsed.name === 'producto');
    assert(
        'describe_model producto tiene columnas y relaciones',
        parsed.columns.length > 0 && parsed.relations.length > 0,
    );
}
...
if (descMd && !descMd.isError) {
    assert(
        'describe_model factura_cliente markdown contiene header',
        descMd.content[0].text.includes('# Modelo `factura_cliente`'),
    );
}
...
assert(
    'verify_model_columns con conexión inexistente devuelve isError',
    verifyNoConn?.isError === true,
);
if (verifyNoConn) {
    assert(
        'mensaje de error menciona el modelo y el endpoint',
        verifyNoConn.content[0].text.includes('cliente') && verifyNoConn.content[0].text.includes('/clientes'),
    );
}
...
assert(
    'resolveModelFromToolName(create_cliente) → cliente',
    enrichInternals.resolveModelFromToolName('create_cliente')?.name === 'cliente',
);
assert(
    'resolveModelFromToolName(update_factura_cliente) → factura_cliente',
    enrichInternals.resolveModelFromToolName('update_factura_cliente')?.name === 'factura_cliente',
);
assert(
    'resolveModelFromToolName(get_clientes) → cliente (vía endpoint)',
    enrichInternals.resolveModelFromToolName('get_clientes')?.name === 'cliente',
);
assert(
    'resolveModelFromToolName(get_facturaclientes) → factura_cliente',
    enrichInternals.resolveModelFromToolName('get_facturaclientes')?.name === 'factura_cliente',
);
assert(
    'resolveModelFromToolName(list_connections) → undefined',
    enrichInternals.resolveModelFromToolName('list_connections') === undefined,
);

// 15. enrichTool: descripción vacía se rellena, descripción existente NO se pisa
const fakeCreateCliente: Tool = {
    name: 'create_cliente',
    description: 'fake',
    inputSchema: {
...
const props = (fakeCreateCliente.inputSchema as { properties: Record<string, { description?: string; maxLength?: number; enum?: string[] }> }).properties;
assert(
    'cifnif: description vacía se rellena del metadata o queda vacía si no hay description',
    typeof props.cifnif?.description === 'string',
);
assert(
    'cifnif: maxLength se añade del metadata (30)',
    props.cifnif?.maxLength === 30,
    `maxLength: ${props.cifnif?.maxLength}`,
);
assert(
    'nombre: description hardcoded NO se pisa',
    props.nombre?.description === 'Nombre o razón social del cliente',
);
assert(
    'nombre: maxLength se añade aunque ya tenía description (100)',
    props.nombre?.maxLength === 100,
    `maxLength: ${props.nombre?.maxLength}`,
);
assert(
    'personafisica tiene boolean (no enum) — el metadata captura enumValues solo de selects',
    props.personafisica?.enum === undefined || (Array.isArray(props.personafisica?.enum) && props.personafisica?.enum.length > 0),
);
assert(
    'connection no se modifica',
    props.connection?.maxLength === undefined,
);

// 16. enrichTool sobre tool sin modelo devuelve false
const orphanTool: Tool = {
...
toolsMap.set('add_connection', {
    name: 'add_connection',
    description: 'noop',
    inputSchema: { type: 'object' as const, properties: { key: { type: 'string' } }, required: [] },
});
...
assert(
    'create_producto: referencia adquiere maxLength 30',
    (toolsMap.get('create_producto')?.inputSchema as { properties: Record<string, { maxLength?: number }> }).properties.referencia?.maxLength === 30,
);
