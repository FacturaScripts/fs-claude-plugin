 ...
 */
export function dateColumnsForTool(toolName: string): string[] {
...
export function dateRangeFilters(
    ...
    if (columnas.length === 0) return out;
...
export function addDateRangeParams(
    ...
    soloEstas?: ReadonlySet<string>,
): number {
    let añadidos = 0;

    for (const tool of tools.values()) {
        if (soloEstas && !soloEstas.has(tool.name)) continue;
        ...
        for (const columna of columnas) {
            for (const { sufijo, texto } of SUFIJOS) {
                const clave = `${columna}${sufijo}`;
                if (schema.properties[clave] !== undefined) continue;
